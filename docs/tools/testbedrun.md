...
4.  **Executes Redeployment:** For *each* instance selected by the user:
    1.  Changes directory to the corresponding Terragrunt subdirectory (e.g., `<rootpath>/terragrunt/repository/` for `reposerver` based on the example YAML).
    2.  Runs `terragrunt destroy --target openstack_compute_instance_v2.<instance_name>` to specifically destroy only that OpenStack compute instance resource. (Example: `terragrunt destroy --target openstack_compute_instance_v2.reposerver`).
    3.  If the destroy command succeeds, it immediately runs `terragrunt apply` within the *same subdirectory* to recreate the instance based on the current configuration.
