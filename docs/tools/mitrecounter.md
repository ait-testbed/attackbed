...
This tool, `tt_counter.py`, reads text files, identifies MITRE ATT&CK® technique IDs (e.g., `T1234`), and counts their occurrences. It utilizes the [mitreattack-python](https://github.com/mitre-attack/mitreattack-python) library to retrieve and display the associated tactics for each identified technique based on the official MITRE ATT&CK CTI data.
