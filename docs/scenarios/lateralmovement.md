...
8.  Lateral Movement via
    1)  share: attacker replaces installation file that is used by another server(T1080)
    2)  exploit: attacker exploits a vulnerable service on another server(T1210)
    3)  apt: attacker replaces debian repository files which gets installed by another server(T1072)
    4)  puppet: attacker deploys backdoor via puppet(T1072)
